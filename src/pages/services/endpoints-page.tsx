...
import { getEndpoints, getEndpointGroups, getEndpointEntities, getEndpointMethods, discoverEndpoints } from '@/services/admin-service';
...
export default function EndpointsPage() {
  ...
  const { data: endpointsResponse, isLoading } = useQuery({
    queryKey: ['admin', 'endpoints', { entity: entityFilter || undefined, group: groupFilter || undefined, method: methodFilter || undefined }],
    queryFn: () => getEndpoints({ entity: entityFilter || undefined, group: groupFilter || undefined, method: methodFilter || undefined }),
  });
  ...
  const { data: entitiesResponse } = useQuery({
    queryKey: ['admin', 'endpoint-entities'],
    queryFn: getEndpointEntities,
  });

  const { data: methodsResponse } = useQuery({
    queryKey: ['admin', 'endpoint-methods'],
    queryFn: getEndpointMethods,
  });

  const entities: string[] = entitiesResponse?.data ?? [];
  const methods: string[] = methodsResponse?.data ?? [];
  ...
  const discoverMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin', 'endpoint-entities'] });
      queryClient.invalidateQueries({ queryKey: ['admin', 'endpoint-methods'] });
