...
// ---------------------------------------------------------------------------
// Helpers (exported for reuse by configuration-page)
// ---------------------------------------------------------------------------
...
/** Known category sort order */
export const KNOWN_CATEGORY_ORDER = Object.keys(CATEGORY_LABELS);
...
export default function SettingsCategoryDetail({ category, settings }: SettingsCategoryDetailProps) {
  ...
  function openCreate() {
    setCreateForm({ ...INITIAL_FORM, key: `${category}.` });
    createModal.open();
  }
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-border bg-surface-raised px-4 py-3">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-semantic-text-default">{getCategoryLabel(category)}</h2>
            <p className="text-xs text-semantic-text-faint mt-0.5">
              {settings.length} setting{settings.length !== 1 ? 's' : ''}
              {isManaged && ' — managed via dedicated settings page'}
            </p>
          </div>
          {!isManaged && (
